...
For CivitAI (required for some models):
  modal secret create civitai-secret CIVITAI_API_KEY=1b87e9cb47383590d31d13117a6b1f45

For HuggingFace (required for gated models):
  modal secret create huggingface-secret HF_TOKEN=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
"""
...
# ── Volume & App ─────────────────────────────────────────────────
CACHE_DIR    = "/vol/models"
CUSTOM_NODES = "/vol/custom_nodes"
CHECKPOINTS  = f"{CACHE_DIR}/checkpoints"

app          = modal.App("alphacore-model-loader")
cache_volume = modal.Volume.from_name("vo-jIx4luG87E8uTijb2Ut8Lu", create_if_missing=False)
...
def clone_custom_node(git_url: str):
    import os
    import subprocess
    from pathlib import Path

    # Use the same volume, just a different path
    dest_dir = Path("/vol/custom_nodes")
    dest_dir.mkdir(parents=True, exist_ok=True)
    ...
    if target_path.exists():
        print(f"⚠  Custom node {repo_name} already exists. Pulling latest...")
        subprocess.check_call(["git", "pull"], cwd=str(target_path))
    else:
        print(f"📥 Cloning {git_url} into {target_path}...")
        subprocess.check_call(["git", "clone", git_url, str(target_path)])

    cache_volume.commit()
    print(f"✅ Custom node {repo_name} installed and volume committed.")
...
""")
    ...
    print(f"    {GREEN}[C]{RESET}  Custom Node (Git Clone)")
    print(f"    {GREEN}[L]{RESET}  List current checkpoints")
    print(f"    {GREEN}[Q]{RESET}  Quit")
    print()

    choice = pick("Enter choice (1/2/3/C/L/Q)", {"1", "2", "3", "C", "c", "L", "l", "Q", "q"}).upper()
    ...
    # ── Custom Node ──────────────────────────────────────────────
    if choice == "C":
        print(f"\n  {BOLD}Custom Node Installation{RESET}")
        print(f"  {DIM}Paste the GitHub repository URL of the custom node.{RESET}\n")
        
        git_url = prompt("Git URL")
        if not git_url:
            print(f"  {RED}No URL provided. Exiting.{RESET}")
            sys.exit(1)
            
        print(f"\n  {YELLOW}Dispatching to Modal...{RESET}\n")
        clone_custom_node.remote(git_url)
        sys.exit(0)
